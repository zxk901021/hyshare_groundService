...
//        }
    ...
    protected BitmapDescriptor getEndBitmapDescriptor() {
        return BitmapDescriptorFactory.fromResource(R.drawable.drawableTrans);
    }
...
//    }
     ...
     * @param height
     */
    public void zoomToSpan(int width, int height) {
        if (startPoint != null) {
            if (mAMap == null)
                return;
            try {
                if (width == -1 || height == -1) {
                    ...
                    LatLngBounds bounds = getLatLngBounds();
                    mAMap.animateCamera(CameraUpdateFactory
                            .newLatLngBounds(bounds, width, height, 150));
                }
            } catch (Throwable e) {
                e.printStackTrace();
            }
